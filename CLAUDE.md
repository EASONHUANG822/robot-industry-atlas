...
- **Application submission**: All applications post to `/api/applications`, which validates fields, checks date availability, and creates records in Airtable (Status="New"). No third-party payment or email integration.
...
- `ApplicationForm` accepts `paymentMode` prop. When `true`, it pre-selects date/time/visitor fields and uses the CheckoutModal ticket flow. When `false`, it renders the full free-form visit application. Both paths post to `/api/applications`.
